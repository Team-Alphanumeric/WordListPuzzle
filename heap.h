...
class heap // make heap a vector with expanded data & functionality
{
		public:
			...
			initializeMaxHeap(vector<T> input) {data=input;} // accepts a vector of T(hings)
			int size() {return data.size();}
			void buildMaxHeap(); // turns a heap structure into a valid heap
			void maxHeapify(int i); // makes a heap valid
			int getHeapSize() {return sortedheapsize;} // get the heap size
			int setHeapSize(int n) {sortedheapsize=n;} // set the heap size
//			void resize(int n) { data.resize(n); } // resize the vector
//			void push_back(T gj) {data.push_back(gj);} // pushes an item to the back of the list
